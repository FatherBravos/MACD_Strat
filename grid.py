...
start_date, end_date = '2023-01-03T0000', '2023-03-30T0000'

grid = {
    ##### point and figure variables ######

    'coin': [('darwinex', 'TSLA','spot','ask'),
             ],

    'timeframe':['1min'],

    'indicators': {'macd': {'timeframe': ['1hour'], 'fast_macd': list(map(int, np.arange(8, 9, 1))),
                            'slow_macd': list(map(int, np.arange(26, 27, 1))),
                            'signal_macd': list(map(int, np.arange(9, 10, 1)))},

                   'atr': {'timeframe': ['1hour'], 'lookback': [14]},

                   'adx': {'timeframe': ['1hour'], 'lookback': list(map(int, np.arange(18, 20, 4)))}
